...
$('#EmailButton').on('click', function () {
    ...
    $.ajax({
        url: 'https://script.google.com/macros/s/AKfycbxsKiCXmBiUZwzHIkBsTW5C8z5wl3SEMDzqQGjklJpSRA_FzXDicTmWmx8liaAAMBDS/exec',
        type: 'POST',
        dataType: 'text',
        data: JSON.stringify({ Name: Email_Name, Address: Email_Address, Contact: Email_Contact }),
        contentType: 'text/plain; charset=utf-8',
        success: function (response) {
            alert(response); // 显示成功或错误消息
        },
        error: function () {
            alert('Failed to write data.'); // 处理错误情况
        }
    });
